...
import it.l_soft.contacto.exceptions.DBInterfaceException;
...
public class DBInterface implements Serializable
{
    ...
    public static ArrayList<?> populateCollection(DBConnection conn, String sql, boolean logStatement, Class<?> objClass) throws Exception
	{
		...
		{
			throw new DBInterfaceException(Utils.LanguageResources.getResource("data.DBConnectionError"));
		}
		...
		{
			DBInterface.disconnect(conn);
			throw new DBInterfaceException(Utils.LanguageResources.getResource("data.QueryExcute") + 
										   " (" + sql + ")");
		}
	...
	public static void insert(DBConnection conn , String idColName, Object objectToInsert) throws Exception
	{
    	String sql = "";
    	/*
    	 * Populating a ResultSetMetaData object to obtain table columns to be used in the query.
		...
		sql += ((DBInterface) objectToInsert).getUpdateStatement(conn, sqlQueryColNames, objectToInsert, idColName);
		conn.executeQuery(sql, logStatement);
	}

	public static int insertAndReturnId(DBConnection conn, String idColName, Object objectToInsert) throws Exception
	{
		int id = -1;
	...
	public static DBConnection connect(ApplicationProperties ap) throws Exception
	{
		return new DBConnection(ap);
	}
	...
	public static void truncate(DBConnection conn, String tableName) throws Exception
	{
		conn.executeQuery("TRUNCATE TABLE " + tableName, logStatement);
	}
}
